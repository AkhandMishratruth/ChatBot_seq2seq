...
def filter_data(q, a):
    ...
    for i in range(len(q)):
        qlen, alen = len(q[i].split(' ')), len(a[i].split(' '))
    ...
    filtered = int((len(q) - len(filtered_q))*100/len(q))
...
def process_data():
    ...
    ques = [ line.lower() for line in ques ]    
    ans = [ line.lower() for line in ans ]

    print('\n:: Sample from ques/ans')
    print(ques[121:125])
    print(ans[121:125])
    ...
    ques = [ filter_line(line, EN_WHITELIST) for line in ques ]
    ans = [ filter_line(line, EN_WHITELIST) for line in ans ]
    ...
    qlines, alines = filter_data(ques, ans)
